...
popul_size_save = MMM.popul_size;
...
for kcnt = 1:MMM.Knum
    ...
    % Begin Genetic K-Means Algorithm (GKM)
    disp(sprintf('GKM Algorithm Entered for k = %0.0f',k))
    ...
    mix_propors = zeros(1,k); genscores = zeros(MMM.num_generns,2); termcount = 0;
    ...
    for gencnt = 1:MMM.num_generns
        ...
        for popcnt = 1:MMM.popul_size
            ...
            % evaluate the total within-cluster variance
            pop_fitness(popcnt) = KMix_TotInClustEUDist(Data,population(popcnt,:));
        end         % chromosomes loop
        [~,minind]= min(pop_fitness);
        genscores(gencnt,:) = [min(pop_fitness),mean(pop_fitness)];
        ...
        for mutcnt = 1:num_mut
            ...
            for datcnt = 1:length(ind)
                ...
                for mixcnt = 1:k
                    dist_from_clusts(mixcnt) = sum((dat - mix_meanvecs(:,:,mixcnt)).^2);
                end % mixtures loop
                % compute mutation probabilities
                dist_from_clusts = max(dist_from_clusts) - dist_from_clusts;
                mutprobs = dist_from_clusts / sum(dist_from_clusts);
        ...
        for mutcnt = 1:num_mut
            % first we need to compute the mean vectors (again . . .)
            for mixcnt = 1:k
                ind = new_pop(popmut(mutcnt),:) == mixcnt;
                mix_meanvecs(:,:,mixcnt) = mean(Data(ind,:),1);
            end     % mixtures loop
        ...
        end         % chromosomes to gkm operate
        ...
        if rem(gencnt,2) == 1
            disp(sprintf('GKM Generation %0.0f of %0.0f: Best Score = %0.4f, %0.0f',gencnt,MMM.num_generns,best_score,termcount));
        end
    ...
    GKM_toc = toc;
    disp(sprintf('GKM Completed in \n\t%1.0f Seconds\n\t%1.4f Minutes\n\t%1.4f Hours',GKM_toc./[1,60,3600]));
    MMM.lblTimInit = [MMM.init_type,sprintf(': %0.2f',GKM_toc/60)];
    MMM.lblTimTot = sprintf('Total (m): %1.4f',etime(clock,MMM.totstt)/60);
    Mixture_DispStatus
    % End Genetic K-means algorithm (GKM)
    ...
    for infcnt = 1:10
        ...
        if (p == 2) && (emconvg == 1)
            ...
            if not(isequal(MMM.optim_type,'EM'))
                plottit = {char(plottit); [objec_func,' Value: ',...
                    num2str(SCORES_CERRS(kcnt,1,infcnt),'%0.4f')]};
            end
            if MMM.data_type == 1   % unknown
                ...
                subplot(1,2,2),PlotBivarClusts(Data,best_clustassigns(:,kcnt,infcnt)),...
                    ...
                    num2str(SCORES_CERRS(kcnt,2,infcnt),'%0.2f%%')]},'interpreter','none')
            end
...
disp(sprintf('Mixture_GKM Completed in \n\t%1.0f Seconds\n\t%1.4f Minutes\n\t%1.4f Hours',tottim./[1,60,3600]));
...
clear ksell popcnt datcnt gencnt cnt GKM_toc best_table emconvg best_k kstt
