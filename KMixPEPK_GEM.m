...
if (nargin ~= 9) || (length(GEMparams) < 6) || (sum(size(population) == [GEMparams(1),n]) ~= 2) || (not(isequal([1:k],reshape(unique(population(:)),1,k))))
    ...
    fprintf('KMixPEPK_GEM: INVALID USAGE-Please read the following instructions!\n'), help KMixPEPK_GEM, return
end
...
for gencnt = 1:num_generns
    ...
    for mutcnt = 1:num_mut
        ...
        for datcnt = 1:length(ind)
            ...
            for mixcnt = 1:k
                if (mix_propors(mixcnt)*n < p)
                    % singleton, let pi be 0
                    posteriors(mixcnt) = 0;
                else
                    % should I do something like save the parms for future use in this loop?
                    posteriors(mixcnt) = mix_propors(mixcnt)*...
                        MPEProdKernelPDF(Data([new_pop(popmut(mutcnt),:) == mixcnt],:),dat,smth);
                end
            end % mixtures loop
...
for mixcnt = 1:k
    ...
    if sum(clust_ys) > 1
        [dens,h,betas] = MPEProdKernelPDF(Data(clust_ys,:),Data(clust_ys,:),smth);
        posteriors(clust_ys,mixcnt) = mix_propors(mixcnt)*dens;
    end
    % get densities for datapoints not in this cluster
    if (sum(clust_no) >= 1) && (sum(clust_ys) > 1)
        posteriors(clust_no,mixcnt) = mix_propors(mixcnt)*MPEProdKernelPDF(...
            Data(clust_ys,:),Data(clust_no,:),smth,h,betas);
    end
end     % mixtures loop
