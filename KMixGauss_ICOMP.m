function ICOMP = KMixGauss_ICOMP(data, labels, smth, ifprob)
%{ 
  ICOMP = KMixGauss_ICOMP(data matrix, mixture assignments, ...
  covariance smoother, problem only)
   This will compute ICOMP for a mixture of normals model.
...
if (length(labels) ~= n) || (nargin ~= 4)
    ...
    fprintf('KMixGauss_ICOMP: INVALID USAGE-Please read the following instructions!\n'), help KMixGauss_ICOMP, return
end
...
penalty = (kcnt*p + kcnt*p*(p+1)/2)*ICOMP_mid - ...
    ((p + 2)*slogdetcov - p*slogknum) - (kcnt*p)*log(2*n);
% finish ICOMP
