function [posteriors,mix_propors,mix_meanvecs,mix_covrmats] = KMixKernel_GEM(Data,GEMparams,bandtype,objec_func,ic,population,smth,savename,init,pltflg)
%{
  [posteriors, mixing proportions, mean vectors, covar matrices] = KMixKernel_GEM(
  data, GA parameters, bandwidth estimator, objec funct, inf_crit_flag, init popul, ...
  covar smoother, figure name, init. method, plot flag)
   This implements the genetic EM algorithm for a mixture of kernel density
...
if (nargin ~= 10) || (length(GEMparams) < 6) || (sum(size(population) == [GEMparams(1),n]) ~= 2) || (not(isequal([1:k],reshape(unique(population(:)),1,k))))
    % wrong # of args, wrong # of GA params, initial population wrong size, not all k represented
    fprintf('KMixKernel_GEM: INVALID USAGE-Please read the following instructions!\n'), help KMixKernel_GEM, return
end
...
% initialize things
mix_propors = zeros(1,k); posteriors = mix_propors; sumks = sum([1:k]); I = zeros(p);
pltxyy = []; genscores = zeros(num_generns,2); best_score = -Inf; termcount = 0;
fhga = figure;
...
for gencnt = 1:num_generns
    ...
    for popcnt = 1:popul_size
        ...
        pop_fitness(popcnt) = ic*feval(objec_func,Data,population(popcnt,:),smth,bandtype,1);
    ...
    for mutcnt = 1:num_mut
        % first we need to compute the pis
        for mixcnt = 1:k
            mix_propors(mixcnt) = sum(new_pop(popmut(mutcnt),:) == mixcnt)/n;
        end     % mixtures loop
        ...
        for datcnt = 1:length(ind)
            ...
            for mixcnt = 1:k
                if (mix_propors(mixcnt)*n <= 1)
                    % singleton, let pi be 0
                    posteriors(mixcnt) = 0;
                else
                    posteriors(mixcnt) = mix_propors(mixcnt)*...
                        MVKDE_Gauss(Data([new_pop(popmut(mutcnt),:) == mixcnt],:),...
                        dat,bandtype,smth,1);
                end
            end % mixtures loop
            ...
            % if not all clusters equally likely, mutate
            if sum(posteriors) ~= 0                
                ...
                if (~isempty(mutprobs)) && (new_pop(popmut(mutcnt),ind(datcnt)) ~= mutprobs)
                    ...
                    % recompute mixing proportions for both affected clusters
                    mix_propors(val) = sum(new_pop(popmut(mutcnt),:) == val)/n;
                    mix_propors(mutprobs) = sum(new_pop(popmut(mutcnt),:) == mutprobs)/n;
                end
            end
        end     % datapoints loop
    ...
    for mutcnt = 1:num_mut
        % first we need to compute the mixing proportions (again ...)
        for mixcnt = 1:k
            mix_propors(mixcnt) = sum(new_pop(popmut(mutcnt),:) == mixcnt)/n;
        end     % mixtures loop
        ...
        for datcnt = 1:n
            ...
            if (sum(posteriors == posteriors(1)) ~= 3)
                ...
                if (new_pop(popmut(mutcnt),datcnt) ~= ind)
                    ...
                    % recompute mixing proportions for both affected clusters
                    mix_propors(val) = sum(new_pop(popmut(mutcnt),:) == val)/n;
                    mix_propors(ind) = sum(new_pop(popmut(mutcnt),:) == ind)/n;
                end
            end
        end     % datapoints loop
...
mix_meanvecs = zeros(1,p,k); mix_covrmats = zeros(p,p,k); 
mix_propors = zeros(1,k); posteriors = zeros(n,k);
for mixcnt = 1:k
    ...
    % compute the posteriors probabilities
    clust_ys = (best_chrom == mixcnt);  % datapoints in this mixture
    clust_no = (best_chrom ~= mixcnt);  % datapoints in this mixture - not!
    % get densities for datapoints in this cluster
    if sum(clust_ys) > 1
        [f,h] = MVKDE_Gauss(Data(clust_ys,:),Data(clust_ys,:),bandtype,smth,1);
        posteriors(clust_ys,mixcnt) = mix_propors(mixcnt)*f;
    end
    % get densities for datapoints not in this cluster
    if (sum(clust_no) >= 1) && (sum(clust_ys) > 1)
        posteriors(clust_no,mixcnt) = mix_propors(mixcnt)*...
...
%{
JAH 20070219, checked for octave 3.4.3
