function [posteriors,mix_propors,mix_meanvecs,mix_covrmats] = KMixMEC_GEM(Data,GEMparams,ECtype,objec_func,ic,population,smth,savename,init,pltflg)
%{
  [posteriors, mixing proportions, mean vectors, covar matrices] = KMixMEC_GEM(
  data, GA parameters, EC type, objec funct, inf_crit_flag, init popul, ...
  covar smoother, figure name, init. method, plot flag)
   This implements the genetic EM algorithm for the EC mixture model.  It
   ...
   GA Parameters --- [population size, number generations, elitism, ...
      xover rate, mutate rate, early termination threshold,EC type]
...
if (nargin ~= 10) || (length(GEMparams) < 6) || (sum(size(population) == [GEMparams(1),n]) ~= 2) ...
        || (not(isequal([1:k],reshape(unique(population(:)),1,k))))
    % wrong # of args, wrong # of GA params, initial population wrong size, not all k represented
    fprintf('KMixMEC_GEM: INVALID USAGE-Please read the following instructions!\n'), help KMixMEC_GEM, return
end
...
mix_ECparms = zeros(1,2+isequal(ECtype,'KT'),k); best_chrom = population(1,:);
...
for gencnt = 1:num_generns
    ...
    for popcnt = 1:popul_size
        ...
        pop_fitness(popcnt) = ic*feval(objec_func,Data,population(popcnt,:),smth,ECtype,1);
    ...
    for mutcnt = 1:num_mut
        ...
        for mixcnt = 1:k
            ...
            if sum(ind) <= 1    % ensure we store cov = zeros(p) for singleton or missing clusters
                ...
                [mix_meanvecs(:,:,mixcnt),mix_covrmats(:,:,mixcnt),mix_ECparms(:,:,mixcnt)]...
                    = MECEstimate(Data(ind,:),ECtype,smth,1);
        ...
        for datcnt = 1:length(ind)
            ...
            for mixcnt = 1:k
                if isequal(mix_covrmats(:,:,mixcnt),I)
                    ...
                    [jnk,posteriors(mixcnt)] = MECPDF(dat,ECtype,mix_meanvecs(:,:,mixcnt),...
                        mix_covrmats(:,:,mixcnt),mix_ECparms(:,:,mixcnt));
                    posteriors(mixcnt) = posteriors(mixcnt)*mix_propors(mixcnt);
                end
            end % mixtures loop
            ...
            if sum(posteriors) ~= 0
                ...
                if (~isempty(mutprobs)) && (new_pop(popmut(mutcnt),ind(datcnt)) ~= mutprobs)
                    ...
                    if mix_propors(val)*n <= 1
                        ...
                        [mix_meanvecs(:,:,val),mix_covrmats(:,:,val),mix_ECparms(:,:,val)]...
                            = MECEstimate(Data(tmp,:),ECtype,smth,1);
                        % if still ill-conditioned, make all zeros
                    ...
                    if mix_propors(mutprobs)*n <= 1
                        ...
                        [mix_meanvecs(:,:,mutprobs),mix_covrmats(:,:,mutprobs),mix_ECparms(:,:,mutprobs)]...
                            = MECEstimate(Data(tmp,:),ECtype,smth,1);
                        % if still ill-conditioned, make all zeros
        ...
        for datcnt = 1:n
            ...
            for mixcnt = 1:k
                if isequal(mix_covrmats(:,:,mixcnt),I)
                    ...
                    [jnk,posteriors(mixcnt)] = MECPDF(Data(datcnt,:),ECtype,mix_meanvecs(:,:,mixcnt),...
                        mix_covrmats(:,:,mixcnt),mix_ECparms(:,:,mixcnt));
                    posteriors(mixcnt) = posteriors(mixcnt)*mix_propors(mixcnt);
                end
            end % mixtures loop
            ...
            if (sum(posteriors == posteriors(1)) ~= k)
                ...
                if (new_pop(popmut(mutcnt),datcnt) ~= ind)
                    ...
                    if mix_propors(ind)*n <= 1
                        ...
                        [mix_meanvecs(:,:,ind),mix_covrmats(:,:,ind),mix_ECparms(:,:,ind)]...
                            = MECEstimate(Data(tmp,:),ECtype,smth,1);
                        % if still ill-conditioned, make all zeros
...
for mixcnt = 1:k
    ...
    if isequal(mix_covrmats(:,:,mixcnt),I) || (MatrixProblem(mix_covrmats(:,:,mixcnt)) ~= 0)
        ...
        [jnk,posteriors(:,mixcnt)] = MECPDF(Data,ECtype,mix_meanvecs(:,:,mixcnt),...
            mix_covrmats(:,:,mixcnt),mix_ECparms(:,:,mixcnt));
        posteriors(:,mixcnt) = posteriors(:,mixcnt)*mix_propors(mixcnt);
