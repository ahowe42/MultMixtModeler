function ICOMP = KMixMEC_ICOMPPEU(data,labels,smth,type,ifprob)
%{
  ICOMP = KMixMEC_ICOMPPEU(data matrix, mixture assignments, covariance smoother,...
  subtype, problem only)
   This will compute ICOMP_PEU for a mixture of elliptically-contoured model.
   ...
   ICOMP --- ICOMP(IFIM)_PEU for the EC mixture model
...
if (length(labels) ~= n) || (nargin ~= 5)
    ...
    fprintf('KMixMEC_ICOMPPEU: INVALID USAGE-Please read the following instructions!\n'), help KMixMEC_ICOMPPEU, return
end
...
elseif isequal(type,'PVII')
    ...
    fprintf('KMixMEC_ICOMPPEU: TYPE INVALID-Please read the following instructions!\n'), help KMixMEC_ICOMP, return
end
...
rnk = kcnt*(1+IFIMp);
penalty = rnk*log(traIFIMs/rnk)-log(detIFIMs);

% finish ICOMP
ICOMP = -2*loglike + penalty;

%{
JAH 20081009, checked for octave 3.4.3
