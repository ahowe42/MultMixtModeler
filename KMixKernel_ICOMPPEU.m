function ICOMPPEU = KMixKernel_ICOMPPEU(data,labels,smth,htype,ifprob)
% ICOMPEU = KMixKernel_ICOMPPEU(data, labels, covariance smoother, ...
...
%  ICOMPPEU --- value of ICOMPPEU(IFIM) for a mixture of kernels model
%
%  JAH 20070220
...
if (nargin ~= 5) || (length(labels) ~= n)
    ...
    fprintf('KMixKernel_ICOMPPEU: INVALID USAGE-Please read the following instructions!\n'), help KMixKernel_ICOMPPEU, return
end
...
m = (kcnt*hest + (kcnt-1)); % number parameters estimated
