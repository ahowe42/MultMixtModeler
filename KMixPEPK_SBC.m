function SBC = KMixPEPK_SBC(data,labels,smth,ifprob)
% SBC = KMixPEPK_SBC(data, labels, covariance smoother,problem only)
%  Compute SBC for a mixture model based on a multivariate product kernel
%  with the PE kernel.
%
%  Where
...
if (nargin ~= 4) || (length(labels) ~= n)
    ...
    fprintf('KMixPEPK_SBC: INVALID USAGE-Please read the following instructions!\n'), help KMixPEPK_SBC, return
end
...
penalty = log(n)*(kcnt*3*p + (kcnt-1));  % (k-1) mixing proportions + mu, h, beta for all dimensions
