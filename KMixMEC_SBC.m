...
if (length(labels) ~= n) || (nargin ~= 5)
    ...
    fprintf('KMixMEC_SBC: INVALID USAGE-Please read the following instructions!\n'), help KMixMEC_SBC, return
end
...
novars = ones(1,kcnt);
if isequal(type,'KT')
    mix_genparms = zeros(1,3,kcnt); % 3 parms for Kotz
elseif isequal(type,'PVII')
    mix_genparms = zeros(1,2,kcnt); % 3 parms for Pearson VII
else
    fprintf('KMixMEC_SBC: TYPE INVALID-Please read the following instructions!\n'), help KMixMEC_SBC, return
end
...
for mixcnt = 1:kcnt
    ...
    mix_parms(:,:,mixcnt) = parms;                      % cluster generator parameters
...
for mixcnt = 1:kcnt    % estimate pi*pdf(data,muk,sigmak)
    if (novars(mixcnt) == 0)
        [loglike,densities(:,mixcnt)] = MECPDF(data,type,mix_meanvecs(:,:,mixcnt),mix_covrmats(:,:,mixcnt),mix_parms(:,:,mixcnt));
